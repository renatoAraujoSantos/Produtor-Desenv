...
export default class InAppPurchases extends React.Component{
  ...
  async prepareAsync() {
    // Este método deve ser chamado primeiro para inicializar os ouvintes e o cliente de faturamento
    await connectAsync();

    /*
      Essas entradas de itens são criadas no App Store Connect e no Google Play Console, respectivamente.
      Se quiser adicionar mais ou editar seus atributos, você pode fazer isso aqui.
    */
    ...
    // Obtenha detalhes do produto
    const { responseCode, results } = await getProductsAsync(items);
    if (responseCode === IAPResponseCode.OK) {
      this.setState({ items: results });
    }

    // Definir ouvinte de compra
