...
import { View, Text, Button, TouchableOpacity, Dimensions, TextInput,
    Platform, StyleSheet, ScrollView, Alert, BackHandler } from 'react-native';
...
export default function CadastroValidarCodigo({ route, navigation }) {

    // const resetNavigation = () => {
    //     navigation.reset({ index: 0, routes: [  { name: 'RootAutenticacao' }  ] });
    // }
    // const backAction = () => {
    //     resetNavigation()
    //     return true;
        ...
        async function fetchData() {
            try {
                const unsubscribe = navigation.addListener('focus', async () => {
                    ...
                    const { codigoValidacao } = route.params;
                    const { dadosUsuarioCadastro } = route.params;

                    if(dadosUsuarioCadastro.email != null){
                        setMessageDestinoCodigo(`O código enviado para o e-mail: ${dadosUsuarioCadastro.email}`);                        
                    }

                    if(dadosUsuarioCadastro.telefoneDestino != null){
                        setMessageDestinoCodigo(`O código enviado via SMS para o nº: ${dadosUsuarioCadastro.celular}`);
                        //setMessageDestinoCodigo(`O código enviado via SMS para o nº: ${'(55)55555-5555'}`);
                    }                    
                    setIdCodigoValidacaoP(codigoValidacao.codigoDescripto);
                });
                setLoading(false);
            } catch (e) {
                setLoading(false);
    ...
    const validarCodigo = async () => {
        ...
        //console.log('5555555555---------------------------------', valueCodigo); 
        //console.log('6666666666---------------------------------', codigoValidacao.codigoDescripto); 

        if(valueCodigo != codigoValidacao.codigoDescripto){
        ...
        const response = await Api.postRequestSemAccessToken(`usuarios/inserirUsuarioSemAccessToken`, `POST`, dadosUsuarioCadastro);
            ...
            <Animatable.View  animation="fadeInUpBig" style={styles.footer} >
                <ScrollView>
                    ...
                    <View style={styles.button}>
                        <TouchableOpacity  style={styles.signIn} onPress={validarCodigo} >
                            <LinearGradient colors={['#56BA42', '#3c812e']} style={styles.signIn} >
                                <Text style={[styles.textSign, { color: '#fff'  }]}>Continuar</Text>
                            </LinearGradient>
                        </TouchableOpacity>

                        <TouchableOpacity onPress={() => navigation.goBack() } style={[styles.signIn, { borderColor: '#3c812e', borderWidth: 1, marginTop: 15   }]} >
